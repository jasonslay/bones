...
/// in a scoring combination (dead dice in the selection).
pub fn score_dice(dice: &[u8]) -> Option<ScoreOutcome> {
    ...
    for face in 1..=6 {
        ...
        match face {
            ...
            _ if c > 0 => return None, // non-scoring leftover
            _ => {}
        }
    }
...
/// Whether a rolled set has at least one legal scoring selection.
pub fn has_any_score(dice: &[u8]) -> bool {
    if dice.is_empty() {
        return false;
    }
    // Quick checks for singles / n-of-a-kind
    let mut counts = [0u8; 7];
    for &d in dice {
        if (1..=6).contains(&d) {
            counts[d as usize] += 1;
...
/// Validate that `selected` indices into `dice` form a complete scoring set
/// (every selected die contributes) and return the outcome.
...
mod tests {
    ...
    fn three_ones() {
        ...
        assert_eq!(score_dice(&[1, 1, 1, 1]).unwrap().points, 1000); // four of a kind
